...
MCE.core = {
    ...
    doConversion(txt, sw) {
        ...
        if (temp.multiplier && temp.multiplier > 1) {
    ...
    preprocessRawText(txt) {
        return String(txt).replace("¼", " 1/4").replace("½", " 1/2").replace("¾", " 3/4");
    },

    convertText(d) {
        var txt = this.preprocessRawText(d.nodeValue);
        if (d.nextSibling != undefined && d.nextSibling.nodeName == 'SUP') {
            var sup_index = txt.length;
            txt = txt + '^' + d.nextSibling.firstChild.nodeValue;
        }
        
        var re = /[0-9]/;
        var result = re.exec(txt);
        if (!result) {
            return false;
        }
		...
		while (wordData = this.getSelectedWord(processText)) {
            wordData = this.getSelectedWord(processText);
            ...
            cutLength += wordData.index + wordData.match.length;
            ...
            if (cr && !cr.inhibit) {
                ...
                crNode.className = "metricationtooltip";
                var crNodeValue = wordData.match + "<span class=\"metricationtooltiptext\">" + cr.inValue + " " + cr.inUnit + " = " + cr.outCustom + "</span>";
                crNode.innerHTML = crNodeValue.trim();
                d.parentNode.insertBefore(crNode, d.splitText(cutLength));
                crNode.previousSibling.nodeValue = crNode.previousSibling.nodeValue.substring(0, crNode.previousSibling.nodeValue.length - wordData.match.length);
                return true;
            }
        }
